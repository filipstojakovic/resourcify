...
public class ResourceReservationService {
  ...
  public ReservationResponse reserveResource(ReserveResourceRequest reserveResourceReq, Jwt jwt) {
    ...
    if (!JwtUtils.hasAdminRole(jwt) && !DateTimeUtils.isAtLeast12HoursFromNow(reserveResourceReq.getFromDate())) {
    ...
    // TODO: filter DECLINED and PENDING
    if (!overlapService.isResourceAvailable(resource.getReservations(),
        reserveResourceReq.getFromDate(),
        reserveResourceReq.getToDate(),
        resource.getAmount())) {
